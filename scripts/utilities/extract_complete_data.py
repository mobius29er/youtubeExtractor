...
def safe_float(value, default=0.0):
    ...
    if isinstance(value, str):
        try:
            ...
            return default
    return default
        missing_channels = set(df['channel_name'].unique()) - set(pd.read_csv('extracted_data/api_only_ml_SAFE.csv') if os.path.exists('extracted_data/api_only_ml_SAFE.csv') else pd.read_csv('extracted_data/api_only_ml_dataset.csv')['channel_name'].unique())
        if missing_channels:
